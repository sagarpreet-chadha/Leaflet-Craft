...
export function fillPolygon(map, polygon, options) {
  ...
  unkinkedPolygons.features.map(feature => {
    createFor(map, feature.geometry.coordinates[0], options, true, pid, 0);
  });
}
...
export default (map, polygons, options) => {
  ...
  if (intersectingTurfPolygons.length) {
    ...
    options.currentOverlappingPolygon &&
      (options.mergedFromPolygons = options.mergedFromPolygons.filter(
        polygon =>
