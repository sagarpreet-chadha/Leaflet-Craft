...
export const mergedPolygonCreatedHandler = data => {
  ...
  data.options.mergedFromPolygons &&
    (mergedPolygonsMap[data.count] = data.options.mergedFromPolygons);
  return;
};
...
export const mergePolygonRedoHandler = data => {
  ...
  undoStackObject[data.pid].push(data.addedPolygons[0]);
  data.options.mergedFromPolygons &&
    (mergedPolygonsMap[data.pid] = data.options.mergedFromPolygons);
  return;
};
...
export const newPolygonCreatedHandler = data => {
  ...
  const limitReached =
    polygons.get(data.map).size === data.options.maximumPolygons;
  // if new Polygon is created and it is intersecting -> do not add to Undo Stack .    ;
  if (
    data.isIntersecting &&
    !limitReached &&
    !data.preventMutations &&
    polygons.get(data.map).size > 1 &&
    data.options.mergePolygons
  ) {
    return;
  }
...
export const undoHandler = map => {
  ...
  const undoPoppedEl =
    id && undoStackObject[id] ? undoStackObject[id].pop() : 0;
  ...
  if (id && !undoStackObject[id].empty()) {
    ...
    createFor(
      map,
      revertedToPoly[rawLatLngKey],
      revertedToPoly._options,
      true,
      id,
      1
    );
    ...
    mergedPolygonsMap[id].forEach(element => {
      createFor(
        map,
        element[rawLatLngKey],
        element._options,
        true,
        element[polygonID],
        3
      ); // Should not Update undoStack State .
    });
  }
...
export const redoHandler = map => {
  ...
  const currentEl =
    id && undoStackObject[id].length() ? undoStackObject[id].top() : 0;
  ...
  if (currentEl) {
    ...
    redoPoppedEl &&
      createFor(
        map,
        redoPoppedEl[rawLatLngKey],
        redoPoppedEl._options,
        true,
        id,
        0
      );
